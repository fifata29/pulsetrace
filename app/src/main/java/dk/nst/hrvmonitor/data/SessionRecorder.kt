...
class SessionRecorder(private val appContext: Context) {
    ...
    private fun buildSummaryJson(
        ...
        if (morphology != null && morphology.isAvailable) {
            ...
            append("    \"lvet_ms\": ${num(morphology.lvetMs)},\n")
            append("    \"max_upstroke_per_sec\": ${num(morphology.maxUpstrokePerSec)},\n")
            append("    \"pulse_amp_variation_pct\": ${num(morphology.pulseAmpVarPct)},\n")
